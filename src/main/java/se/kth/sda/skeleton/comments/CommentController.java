...
public class CommentController {
    ...
    // Creates a comment on an id-defined post
    @PostMapping("/postId")
    ...
    // Returns all comments on an id-defined post
    @GetMapping("/postId")
    public ResponseEntity<List<javax.xml.stream.events.Comment>> listAllCommentsOnAPost(@PathVariable Long postId){
        Post post = postRepository.findById(postId).orElseThrow(ResourceNotFoundException::new);

        return ResponseEntity.ok(post.getComments());
    }

    // Updates an id-defined comment
